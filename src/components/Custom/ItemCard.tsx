...
function ItemCard({id,name,price,discount,image}:{id:string,name:string,price:number,discount:number,image:string}) {
  ...
  return (
    <div className="md:w-[280px] w-[150px] lg:w-[350px] lg:h-[400px]  md:h-[350px] h-[260px] min-h-max border bg-slate-50 shadow  rounded md:pt-2 flex flex-col" onClick={()=>{router.push(`/shop/${id}`)}}>
      <Image
        src={
        ...
        height={1000}
        width={1000}
        className="w-full md:h-[250px] h-[140px] object-cover object-center "
        alt="Failed To load"
      />
      <div className="body  grow md:px-0.5  my-2 flex flex-col mt-3">
        {/* <h1 className="text-sm text-neutral-400">Mens</h1> */}
        <div className="card-header flex justify-between items-start gap-2">
          <h1 className="md:text-md md:px-4 px-1 md:text-base text-xs  grow text-wrap">{name}</h1>
          {/* <FaHeart
            className={`${
        ...
        <div className="card-body md:px-4 px-1  mt-1 self-baseline">
